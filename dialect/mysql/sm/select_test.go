...
func TestSelectBasic(t *testing.T) {
	expectedQuery := "SELECT DISTINCT u.id, u.name, ua.address FROM users AS u INNER JOIN users_address AS a ON users.id = a.user_id WHERE age < 10 ORDER BY name ASC OFFSET ? LIMIT ?"
...
func TestSelectBasicArgs(t *testing.T) {
	expectedQuery := "SELECT DISTINCT u.id, u.name, ua.address FROM users AS u INNER JOIN users_address AS a ON users.id = a.user_id WHERE age < ? ORDER BY name ASC OFFSET ? LIMIT ?"
...
func TestSelectApply(t *testing.T) {
	...
	query := mysql.Select(
		Columns("id", "name"),
		Apply(func(a mysql.SelectModApply) {
			a.Apply(
				Where("age < 10"),
				OrderBy("name"),
			)
